...
class FeatureFusion(nn.Module):
    ...
    def align_features(self, features, input_name):
        ...
        if input_name not in self.conv_layers:
            self.conv_layers[input_name] = nn.Conv2d(channels, self.target_channels, kernel_size=1)
        ...
        if (height, width) != self.target_size:
            aligned_features = F.interpolate(aligned_features, size=self.target_size, mode="bilinear")
    ...
    def forward(self, feature1, feature2):
        ...
        mha1 = nn.MultiheadAttention(embed_dim=self.target_channels, num_heads=8)
        mha2 = nn.MultiheadAttention(embed_dim=self.target_channels, num_heads=8)

        # Reshape for attention input (batch_first=True)
        batch_size, channels, height, width = feature1.size()
        feature1_flatten = feature1.view(batch_size, channels, -1).permute(0, 2, 1)  # [batch, seq_len, channels]
        ...
        fused_features = fused_features.permute(0, 2, 1).view(batch_size, self.target_channels, height, width)
