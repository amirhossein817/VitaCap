...
class EncoderModule(nn.Module):
    def __init__(self, target_channels=512, target_size=(20, 20)):
        super(EncoderModule, self).__init__()
        self.target_channels = target_channels
        self.target_size = target_size

        # Feature fusion module
        self.feature_fusion_left = FeatureFusion(target_channels, target_size)
        self.feature_fusion_right = FeatureFusion(target_channels, target_size)
        self.feature_fusion_final = FeatureFusion(target_channels, target_size)
    ...
    def forward(self, feature_swin, feature_yolo, feature_rcnn):
        ...
        fused1 = self.feature_fusion_left(feature_swin, feature_yolo)

        # Fuse Swin and Faster R-CNN features
        fused2 = self.feature_fusion_right(feature_swin, feature_rcnn)
